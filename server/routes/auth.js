...
import { readUsers, writeUsers } from '../controllers/userStore.js'
...
router.post('/password/complete', resetRateLimit, asyncRoute(async (req, res) => {
  ...
  delete users[idx].password // clean up old field if present
  await writeUsers(users)
...
router.post('/email/verify-check', resetRateLimit, asyncRoute(async (req, res) => {
  ...
  const users = await readUsers()
  const idx = users.findIndex(u => u.email === safeEmail)
  if (idx !== -1) { users[idx].emailVerified = true; await writeUsers(users) }
